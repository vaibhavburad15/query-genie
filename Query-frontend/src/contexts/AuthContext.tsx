...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const login = async (credentials: { identifier: string; password: string }): Promise<boolean> => {
    ...
    try {
      ...
      if (response.success) {
        // For now, create a basic user object from the response
        // In a real app, the backend would return user data
        const userData: User = {
          id: Date.now(), // Temporary ID
          email: credentials.identifier.includes('@') ? credentials.identifier : `${credentials.identifier}@example.com`,
          firstName: 'User', // Would come from backend
          lastName: '',
          username: credentials.identifier.includes('@') ? credentials.identifier.split('@')[0] : credentials.identifier,
          contactNumber: '',
