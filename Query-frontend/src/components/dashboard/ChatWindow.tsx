           ...
           variant="destructive"
            ...
            <Button
              ...
              if (sqlMatch && parsedOutput) {
                ...
                if (parsedOutput.type === 'select' && parsedOutput.data && parsedOutput.data.length > 0) {
                  const columns = parsedOutput.columns || extractColumnsFromSql(sqlMatch[1], parsedOutput.data);
                  ...
                  return (
                    <div key={message.id} className="space-y-4">
                      ...
                      <div className="bg-red-50 border border-red-200 text-red-800 px-6 py-4 rounded-lg max-w-[90%] flex items-start gap-3 shadow-sm">
                        ...
                        <div>
                          <p className="font-medium">{parsedOutput.message || 'An error occurred'}</p>
                        </div>
                      </div>
                    </div>
                  );
