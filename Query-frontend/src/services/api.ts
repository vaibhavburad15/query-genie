 ...
 * @param session - The chat session data including title and messages.
 * @returns A promise that resolves with the created chat session.
 */
export const createChatSession = async (session: { title: string; messages: ChatMessage[] }) => {
  try {
 ...
 * @param updatedSession - The updated session data including messages.
 * @returns A promise that resolves with the updated chat session.
 */
export const updateChatSession = async (sessionId: number, updatedSession: { title?: string; messages: any[] }) => {
  try {
